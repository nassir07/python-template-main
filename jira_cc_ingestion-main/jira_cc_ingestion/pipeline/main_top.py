...
from jira_cc_ingestion.config import TOP_PROJECT  # noqa: E402
...
def main(token_auth, user_sql, password_sql):
    """main"""
    # treat CRMST project
    logger.info(f"treating {TOP_PROJECT['name']} project...")
    ingest_project_data(token_auth, user_sql, password_sql, TOP_PROJECT)
    logger.info(f"{TOP_PROJECT['name']} project is treated successfully!")
