...
from jira_cc_ingestion.config import PVI_PROJECT  # noqa: E402
...
def main(token_auth, user_sql, password_sql):
    """main"""
    # treat IPBM project
    logger.info(f"treating {PVI_PROJECT['name']} project...")
    ingest_project_data(token_auth, user_sql, password_sql, PVI_PROJECT)
    logger.info(f"{PVI_PROJECT['name']} project is treated successfully!")
